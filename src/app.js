...
import { City, mulberry32, ROAD_Y, WALK_Y, cropZoneData, WORLD_ANCHOR, WORLD_RADIUS, PLAY_RADIUS } from './citygen.js?v=20260714b';
import { BUILDING_CHUNK_SIZE, buildBuildingGeometry, buildBuildings, buildRoads, buildParks } from './citymesh.js?v=20260714b';
import { buildOjedaStorefront } from './ojeda.js?v=20260714b';
import { buildParkBooth } from './parkbooth.js?v=20260714b';
import { GrassSystem } from './veg/grass.js?v=20260714b';
import { buildFlowerTuft } from './veg/flowers.js?v=20260714b';
import { Player } from './player.js?v=20260714b';
import { MiniMap } from './minimap.js?v=20260714b';
import { StreetLife } from './npcs.js?v=20260714b';
import { sanitizeImported } from './glbutil.js?v=20260714b';
import { buildToonLamp, buildToonBench, buildToonHydrant, buildToonBin, buildToonStreetSign, buildToonPlanter } from './props.js?v=20260714b';
import { Net } from './net.js?v=20260714b';
import { ChatUI, showBubble } from './chat.js?v=20260714b';
import { CLASS_LIST, CERNUNNOS, classById } from './rpg/classes.js?v=20260714b';
import { composeCharacter, sanitizeCustom, defaultCustom, RIGS, RIG_IDS, ACCESSORIES, ACC_IDS, PALETTES_BY_CLASS } from './rpg/charcustom.js?v=20260714b';
import { equipWeapon } from './weapons.js?v=20260714b';
import { authRequest, privyAuthRequest, loadPrivy, PRIVY_APP_ID } from './rpg/account.js?v=20260714b';
import { MobField, warmMobAssets } from './rpg/mobs.js?v=20260714b';
import { Inventory } from './rpg/loot.js?v=20260714b';
import { HUD, Progress, QuestLog, hpMaxForLevel, xpNextForLevel } from './rpg/hud.js?v=20260714b';
import { Combat } from './rpg/combat.js?v=20260714b';
import { applyWeaponTier, makeCharAura, updateAura } from './rpg/fx.js?v=20260714b';
import { Effects } from './rpg/effects.js?v=20260714b';
import { attachWeaponByName } from './weapons.js?v=20260714b';
import { createTextureKit, createToonSkyTexture, createGroundVariationTexture } from './worldmat.js?v=20260714b';
import { buildPoiSigns, installPoiInteractions, loadPublicPois } from './pois.js?v=20260714b';
import { createTrailerMode, createTrailerNet, getTrailerAuth, getTrailerChoice, getTrailerConfig } from './trailer.js?v=20260714b';
import { SocialPanel, showSocialInvite } from './social.js?v=20260714b';
import { SkillSystem } from './rpg/skills.js?v=20260714b';
import { goldRewardMultiplier, materialGoldValue, rollDrops, Wallet } from './rpg/economy.js?v=20260714b';
import { createSfx } from './sfx.js?v=20260714b';
import { installTouchControls } from './touch.js?v=20260714b';
import { createIntroScene } from './introscene.js?v=20260714b';
import { styleCarShell } from './carstyle.js?v=20260714b';
import { actionLabel, createKeybindsPanel, keybindChangeEvent, matchesAction } from './keybinds.js?v=20260714b';
import { FrameMeter, fpsBand } from './perf.js?v=20260714b';
...
async function boot() {
  ...
  buildParkBooth(scene, -77.3, -49.5, 0.42);  // caseta del vigilante en la esquina del parque
  setProgress(0.7);
  await breathe();
...
transformed.xz += vec2( sin( fPh ), cos( fPh * 0.83 ) ) * max( position.y, 0.0 ) * 0.006 * fGust;
  ...
  // MODO DIOS: lo decide el SERVER (cuenta con `god: true`). Da inmortalidad, ropa
  // holografica verde y el kit de RAYOS, sin cambiarte de clase ni de personaje.
  const GODMODE = !!auth.godMode;
  const skills = new SkillSystem(
    choice.god ? 'cernunnos' : (choice.className || 'verdugo'),
    undefined,
    { godMode: GODMODE },
  );
  ...
  // luz azul del recall: PERSISTENTE en la escena (intensity 0 en reposo). Crear o
  // quitar una luz por cada recall recompilaria todos los shaders iluminados. Aqui
  // solo se reposiciona y se pulsa su intensidad; nunca se agrega ni se quita.
  const recallLight = new THREE.PointLight(0x4fb8ff, 0, 9, 2);
  recallLight.position.set(0, -1000, 0);
  scene.add(recallLight);
  ...
  const teleportTick = (dt) => {
    ...
    if (aura) {
      ...
      // la luz azul crece con el progreso del canaleo (luz persistente, sigue al heroe)
      recallLight.position.set(player.pos.x, 1.4, player.pos.z);
      recallLight.intensity = (1.5 + prog * 3) * pulse;
    }
    if (teleCh <= 0) {                                       // fin del channel: tepea
      ...
      recallLight.intensity = 0;                             // apagar SIN quitar de la escena
      player.locked = false;
    }
  };
  if (P.landmark) {
    addEventListener('keydown', (e) => {
      ...
      // el aura additive (cyl+ring) usa MeshBasicMaterial: no participa en la
      // iluminacion, se puede crear/quitar sin recompilar shaders. La luz real la
      // aporta recallLight, que es persistente.
      aura.add(cyl, ring);
      aura.position.set(player.pos.x, 0, player.pos.z);
      recallLight.position.set(player.pos.x, 1.4, player.pos.z);
      scene.add(aura);
  ...
  renderer.setAnimationLoop(() => {
    ...
    if (streetT <= 0) {
      ...
      if (dOj < 9 && !nearOjeda) {
        ...
        nearOjeda = false;
        inventory.setShop([]);
        if (inventory.isOpen()) setInventoryOpen(false);  // al alejarte de la bodega, cierra solo
      }
