...

{
  ...
  // El pool fijo de luces vive en la escena toda la sesion; medimos el aporte
  // propio de cada efecto contra esta base, no contra una escena vacia.
  ...
  if (scene.children.length !== base + 72) throw new Error(`flash scene cap mismatch after hitFlash: ${scene.children.length}`);
  ...
  if (scene.children.length !== base + 72) throw new Error(`flash scene cap mismatch after trail puffs: ${scene.children.length}`);
  ...
  if (scene.children.length !== base) throw new Error(`flash sprites were not removed: ${scene.children.length}`);
...

{
  ...
  if (scene.children.length !== base + 36) throw new Error(`blood pool scene cap mismatch: ${scene.children.length}`);
  ...
  if (scene.children.length !== base) throw new Error(`blood pool meshes were not removed: ${scene.children.length}`);
...

{
  ...
  if (scene.children.length !== base) throw new Error(`blood drip meshes were not removed: ${scene.children.length}`);
...

{
  ...
  if (scene.children.length !== base + 60) throw new Error(`projectile scene cap mismatch: ${scene.children.length}`);
